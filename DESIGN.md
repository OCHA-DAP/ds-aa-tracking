...
| 1 | CI check: every crosswalk row must exist in `framework_version` (catches drift both ways) | ds-knowledge-base PR | Still open |
| 2 | ~~Extract `trigger_source_crosswalk`; `framework_version_map` becomes a compatibility VIEW; `load_aa_performance.py` re-pointed~~ **done (2026-09, KB PRs #608/#609)** | ds-knowledge-base | Done — consumers work via the view |
